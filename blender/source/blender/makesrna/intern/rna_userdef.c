...
static PointerRNA rna_UserDef_vr_get(PointerRNA *ptr)
{
  return rna_pointer_inherit_refine(ptr, &RNA_PreferencesVR, ptr->data);
}
...
static void rna_def_userdef_vr(BlenderRNA *brna)
{
  PropertyRNA *prop;
  StructRNA *srna;

  static const EnumPropertyItem vr_devices[] = {
    {0, "AUTO", 0, "Auto", "Auto-detect connected hardware"},
    {VR_DEVICE_TYPE_OCULUS, "OCULUS", 0, "Oculus", "Oculus Rift and Rift S"},
    {VR_DEVICE_TYPE_VIVE, "VIVE", 0, "Vive", "HTC Vive and Vive Pro"},
    {VR_DEVICE_TYPE_WINDOWSMR, "WINDOWS_MR", 0, "WindowsMR", "Windows Mixed Reality"},
    {VR_DEVICE_TYPE_FOVE, "FOVE", 0, "Fove", "Fove 0"},
    {VR_DEVICE_TYPE_PIMAX, "PIMAX", 0, "Pimax", "Pimax HMDs"},
    {VR_DEVICE_TYPE_INDEX, "INDEX", 0, "Index", "Valve Index"},
    {VR_DEVICE_TYPE_MAGICLEAP, "MAGIC_LEAP", 0, "MagicLeap", "Magic Leap One"},
    {0, NULL, 0, NULL, NULL},
  };

  srna = RNA_def_struct(brna, "PreferencesVR", NULL);
  RNA_def_struct_sdna(srna, "UserDef");
  RNA_def_struct_nested(brna, srna, "Preferences");
  RNA_def_struct_clear_flag(srna, STRUCT_UNDO);
  RNA_def_struct_ui_text(srna, "VR", "Default VR Settings");

  /* General  */

  prop = RNA_def_property(srna, "vr_device", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, NULL, "vr_device");
  RNA_def_property_enum_items(prop, vr_devices);
  RNA_def_property_ui_text(prop, "Device", "Which VR headset to use");

  /* Network */

  prop = RNA_def_property(srna, "vr_network_ip_address", PROP_STRING, PROP_DIRPATH);
  RNA_def_property_string_sdna(prop, NULL, "vr_network_ipaddr");
  RNA_def_property_ui_text(prop, "IP Address", "IPv4 address for VR remote streaming");

  /* Experimental */

  prop = RNA_def_property(srna, "vr_openxr", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, NULL, "vr_openxr", 0);
  RNA_def_property_boolean_default(prop, false);
  RNA_def_property_ui_text(prop, "OpenXR", "Use OpenXR runtime");
}
...
#  endif
  ...
  prop = RNA_def_property(srna, "vr", PROP_POINTER, PROP_NONE);
  RNA_def_property_flag(prop, PROP_NEVER_NULL);
  RNA_def_property_struct_type(prop, "PreferencesVR");
  RNA_def_property_pointer_funcs(prop, "rna_UserDef_vr_get", NULL, NULL, NULL);
  RNA_def_property_ui_text(prop, "VR", "Default VR settings");
