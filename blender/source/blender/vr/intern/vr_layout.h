...
#ifndef __VR_LAYOUT_H__
#define __VR_LAYOUT_H__
...
public:
	...
	static Coord3Df button_positions[VR_DEVICE_TYPES][VR_SIDES][BUTTONIDS];	/* 3D positions of the buttons with respect to the controllers. */

	VR_Layout();				/* Constructor. */
	virtual ~VR_Layout();	/* Destructor. */
public:
	/* UI Layout: mapping of controller functions to widgets. */
	typedef struct Layout {
		std::string name;	/* Name of the mapping. */
		VR_Device_Type type;	/* The device type of the layout. */
		VR_Widget* m[VR_SIDES][BUTTONIDS];	/* Mapping of buttons to widgets (one for each controller side, each button). */
		ButtonBit ctrl_button_bits[VR_SIDES];	/* Controller button bits defined to be the 'ctrl buttons' (per side). */
		ButtonBit shift_button_bits[VR_SIDES];	/* Controller button bits defined to be the 'shift buttons' (per side). */
	...
	static std::vector<Layout*>	layouts[VR_DEVICE_TYPES];	/* List of all layouts (mappings). */

	static const Layout default_layouts[VR_DEVICE_TYPES];	/* Default layout mapping. */
	...
	static VR_UI::Error   map_widget(VR_Side side, std::string event, std::string widget);	/* Set a button-to-widget mapping. */
	static VR_UI::Error   unmap_widget(const std::string& widget);	/* Unmap widget everywhere. */
	static VR_UI::Error   get_mapped_widget(VR_Side side, const std::string& event, std::string& widget);	/* Get a single button-to-widget mapping (widget name). */
	static VR_UI::Error   get_mapped_event(const std::string& widget, VR_Side& side, std::string& event);	/* Get a single button-to-widget mapping (event name). */
	static VR_UI::Error   get_mapped_event(const VR_Widget* widget, VR_Side& side, std::string& event);	/* Get a single button-to-widget mapping (event name). */
	static VR_UI::Error   get_mapped_event(const Layout* layout, const std::string& widget, VR_Side& side, std::string& event);	/* Get a single button-to-widget mapping (event name). */
	static VR_UI::Error   get_mapped_event(const Layout* layout, const VR_Widget* widget, VR_Side& side, std::string& event);	/* Get a single button-to-widget mapping (event name). */
	static VR_UI::Error   get_mapped_event(const Layout* layout, const VR_Widget* widget, VR_Side& side, ButtonID& event);	/* Get a single button-to-widget mapping (event name). */
...
inline VR_Layout::ButtonBit& operator|=(VR_Layout::ButtonBit& b0, const VR_Layout::ButtonBit& b1)
{
	return b0 = VR_Layout::ButtonBit(b0 | b1);
};
inline VR_Layout::ButtonBit operator|(const VR_Layout::ButtonBit& b0, const VR_Layout::ButtonBit& b1)
{
	return VR_Layout::ButtonBit((uint64_t)(b0) | (uint64_t)(b1));
};

#endif /* __VR_LAYOUT_H__ */
