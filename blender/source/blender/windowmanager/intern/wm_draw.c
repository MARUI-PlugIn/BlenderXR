    ...
    case SPACE_VIEW3D: {
      if (ar->regiontype == RGN_TYPE_WINDOW) {
        ...
        if (rv3d->rflag & RV3D_IS_VR) {
          ...
          /* Disable "lock view to XX". */
          v3d->ob_centre = NULL;
          BLI_strncpy_utf8(v3d->ob_centre_bone, "", 64);
          v3d->ob_centre_cursor &= ~1;
          /* Hide text overlays. */
...
static void wm_draw_region_buffer_free(ARegion *ar)
{
   ...
   * is uninitialized. */
  RegionView3D *rv3d = ar->regiondata;
  /* TODO_XR: The "RV3D_IS_VR" test fails if rv3d->rflag
   * is filled with garbage data when the struct is initialized... */
  if (rv3d && (rv3d->rflag & RV3D_IS_VR)!=0 && (rv3d->rflag & 0xF000)==0) {
