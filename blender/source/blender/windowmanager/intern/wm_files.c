...
{
#if WITH_VR
  /* Don't allow saving the scene if a VR window is open.
   * This is a temporary workaround to prevent the VR flags
   * (V3D_S3D_DISPVR, RV3D_IS_VR) from being saved with the scene. */ 
  VR *vr = vr_get_obj();
  if ((vr->ctx == C) && vr->window) {
    BKE_reportf(op->reports, RPT_ERROR, "VR window is open. Scene not saved.");
    return OPERATOR_CANCELLED;
  }
#endif
